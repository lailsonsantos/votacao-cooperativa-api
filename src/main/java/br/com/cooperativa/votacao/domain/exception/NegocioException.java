 ...
 *
 * <p>Cada subclasse declara a <em>natureza</em> da falha ({@link TipoErro}) e um codigo estavel,
 * ambos em vocabulario de dominio. Nenhuma delas conhece HTTP: a traducao para status acontece na
 * camada de API, que e onde HTTP significa alguma coisa.
 *
 * <p>O tratador global apenas transporta esses dados, sem conhecer regra alguma. Assim, uma regra
 * nova nao exige alteracao no tratador, e a decisao sobre a natureza do erro fica junto da regra
 * que a motiva.
 *
 * <p>Falhas de negocio nao sao erros de sistema: um voto duplicado significa que a aplicacao
 * funcionou corretamente ao recusa-lo. Por isso sao registradas em {@code WARN}, nunca em {@code
 * ERROR}.
 */
@Getter
